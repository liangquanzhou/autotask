...
`exec <name>` is the launchd wrapper used for scheduled tasks generated by `sync --apply`. It runs the registered command and records exit status, start/end time, and duration in `~/.config/autotask/runs/<name>.jsonl`. Existing LaunchAgents start recording history only after they are regenerated with `sync --apply`.

`runs <name>` shows recent recorded runs:
